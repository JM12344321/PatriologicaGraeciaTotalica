...
Completed volumes are published as one Markdown file per volume, named `PL_NNN_English.md`. A volume appears only after every source chunk has been translated and independently source-reviewed. Drafts, corrections, and independent source reviews use GPT-5.6 Luna, Terra, and Sol. Requested model routes and review records are retained for each internal section. The internal chunk boundaries are removed from the reading text when the volume is assembled.
